...
public class WidgetListServerPlacements extends WidgetListBase<PlayerPlacementInfo, WidgetServerPlacementEntry> {
    ...
    private static final Comparator<PlayerPlacementInfo> COMPARATOR =
            Comparator.comparingLong(PlayerPlacementInfo::getTimestamp).reversed()
                    .thenComparing(PlayerPlacementInfo::getOwnerName, String.CASE_INSENSITIVE_ORDER);
    ...
    protected List<String> getEntryStringsForFilter(PlayerPlacementInfo entry) {
        ...
        list.add(entry.getPos().toShortString());
        if (entry.getTimestamp() > 0) {
            list.add(WidgetServerPlacementEntry.DATE_FORMAT.format(new Date(entry.getTimestamp())));
        }
        return list;
    }
        ...
        if (filter != null && !filter.trim().isEmpty()) {
            ...
            for (PlayerPlacementInfo p : all) {
                if (p.getSchematicId().toLowerCase(Locale.ROOT).contains(search)
                        ...
                        || (p.getTimestamp() > 0 && WidgetServerPlacementEntry.DATE_FORMAT.format(new Date(p.getTimestamp())).contains(search))) {
                    filtered.add(p);
                }
            }
